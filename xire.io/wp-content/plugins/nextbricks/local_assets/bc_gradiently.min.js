...
class NeatGradient {
  constructor(config) {
    ...
    this._ref = ref;
    this._speed = -1;
    this._horizontalPressure = -1;
    this._verticalPressure = -1;
    this._waveFrequencyX = -1;
    this._waveFrequencyY = -1;
    this._waveAmplitude = -1;
    this._shadows = -1;
    this._highlights = -1;
    this._saturation = -1;
    this._brightness = -1;
    this._grainScale = -1;
    this._grainIntensity = -1;
    this._grainSparsity = -1;
    this._grainSpeed = -1;
    this._colorBlending = 0;
    this._colors = [];
    this._wireframe = !1;
      ...
      meshes.forEach((mesh) => {
        ...
        const colors = [
          ...this._colors.map((color) => {
            ...
            return {
              is_active: color.enabled ? 1.0 : 0.0,
              color: threeColor,
              influence: color.influence,
            };
          }),
          ...Array.from({ length: COLORS_COUNT - this._colors.length }).map(
            () => ({ is_active: 0.0, color: new THREE.Color(0x000000) })
          ),
        ];
        tick += this.clock.getDelta() * this._speed;
        mesh.material.uniforms.u_time.value = tick;
        mesh.material.uniforms.u_resolution.value = new THREE.Vector2(
          width,
          height
        );
        mesh.material.uniforms.u_color_pressure.value = new THREE.Vector2(
          this._horizontalPressure,
          this._verticalPressure
        );
    ...
    const setSize = () => {
      ...
      this.sceneState.renderer.setSize(width, height, !1);
  ...
  set waveAmplitude(waveAmplitude) {
    this._waveAmplitude = waveAmplitude * 0.75;
  }
  set colors(colors) {
    this._colors = colors;
  }
  ...
  set colorBlending(colorBlending) {
    this._colorBlending = colorBlending ? colorBlending / 10 : 0.5;
  }
  ...
  _initScene(resolution) {
    const width = this._ref.width,
      height = this._ref.height;
    const renderer = new THREE.WebGLRenderer({ alpha: !0, canvas: this._ref });
    ...
    return { renderer, camera, scene, meshes, resolution };
  }
  _buildMaterial(width, height) {
    ...
    const uniforms = {
      ...
      u_colors_count: { value: COLORS_COUNT },
...
function updateCamera(camera, width, height) {
  ...
  const targetPlaneArea =
    ((areaViewPort / viewPortAreaRatio) * PLANE_WIDTH * PLANE_HEIGHT) / 1.5;
